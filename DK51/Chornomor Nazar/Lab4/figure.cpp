...
void drawFigure(Figure_s* figure){
    // Малювання овалу за допомогою полігону
    glBegin(GL_POLYGON);
        glColor3f(figure->color->R, figure->color->G, figure->color->B);
        
		float Const_Rad = M_PI / 180.0f;
        // Малюємо овал, проходячи 360 градусів
        for(int i = 0; i <= 360; i += 5) {
            float rad = i * Const_Rad;
            float x = (figure->center->x + figure->rx * cos(rad)) / 100.0f;
            float y = (figure->center->y + figure->ry * sin(rad)) / 100.0f;
            glVertex2f(x, y);
        }
    glEnd();
}
...
int addFigToFigList(FigureList_s *list, Figure_s* figure) {
	if (NULL == list || (list->head == NULL && list->head != list->tail)) return 0;
...
void removeLastElementFromFigList(FigureList_s *list) {
	if (NULL == list || (list->head == NULL && list->head != list->tail)) return;
...
int insertFigToFigListAtIndex(FigureList_s *list, int index, Figure_s* figure) {
	...
	if (count == index) return addFigToFigList(list, figure);
