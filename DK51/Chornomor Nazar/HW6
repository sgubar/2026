...
void display()
{
    glClearColor(1.0f, 0.0f, 0.0f, 1.0f); // Фон чорний та прозорий
    glClear(GL_COLOR_BUFFER_BIT); // Очищення буферу кольору

    // Малювання серії відрізків, що є графіком функції (ВАРІАНТ 12)
    glBegin(GL_LINE_STRIP); // Кожна пара вершини задає відрізок
        glColor3f(0.0f, 0.0f, 0.0f); // Жовтий колір
        for (float x = 0; x <= M_PI * 5; x += 0.1) {
            float y1 = 0.25 * x * (sin(x) / M_PI);
            glVertex2f(x / (M_PI * 5), y1);
        }
    ...
    glBegin(GL_LINE_STRIP); // Кожна пара вершини задає відрізок
        ...
        for (float x = 0; x <= M_PI * 5; x += 0.1) {
            float y2 = 0.25 * x * (cos(x) / M_PI);
            glVertex2f(x / (M_PI * 5), y2);
        }
...
int main(int argc, char* argv[])
{
    ...
    glutCreateWindow("Варіант 12"); // Створення вікна із заданим ім'ям
